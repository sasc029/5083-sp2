...
public class DdmLifeCycleRuleImpl extends LifeCycleRuleImpl {
	...
	public void calculate(Persistable target, String typeId, Context context,
			Map<String, String> args) {
		LOG.debug(LogUtil.beginMethod("正在设置生命周期模板", "目标对象=" + target,
				"typeId=" + typeId, "context=" + context, "args=" + args));
		...
		List<LifeCycleTemplate> templates = DdmSelfDefHelper.getTemplateService()
				.findLifeCycleTemplatesEnabled(context, target.getClassId());
		if (templates.size() == 0) {
		...
		if (stateName != null) {
			// 在目标模板里找给定的状态
			List<State> templateStates = Helper.getLifeCycleService()
					.findStates(lifeCycleTemplate);
