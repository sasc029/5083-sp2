...
<%
	...
	String spot = "ListRecInfos";
	String gridId = "RecInfoGrid";
	String gridTitle = "回收信息";
	String toolbarId = "ddm.recinfo.list.toolbar";
...
<script>
	...
	function addRecInfos(){
		...
		if(treeSelections == null || treeSelections == ""){
			...
			//var url = "<%=contextPath%>/ddm/distribute/recDesInfo!getAllNeddRecInfos.action";
			var url = "<%=contextPath%>/ddm/recdesinfo/addRecInfos.jsp";
			//plm.OpenWindowAndReload(url);
			var windowid = "addRecInfos" + "_" + new Date().getTime();
			plm.openWindow(url , 800, 620, windowid);
		}
	}
	//编辑回收单信息
	function editRecInfos(){
		...
		//window.location.href = "<%=contextPath%>/ddm/recdesinfo/recInfo_content_edit.jsp?oid="+oid;
		var url = "<%=contextPath%>/ddm/recdesinfo/recInfo_content_edit.jsp?oid="+oid;
		plm.OpenWindowAndReload(url);
	}

	// 刷新数据
	function reload() {
		window.location.href = "<%=contextPath%>/ddm/distribute/recDesInfo!getRecInfoByLinkOids.action";
		/*
		var table = pt.ui.get("<%=gridId%>");
		table.reload();
		*/
	...
	function deleteRecInfos(){
		url = "<%=contextPath%>/ddm/distribute/recDesInfo!deleteRecDesInfos.action";
		var table = pt.ui.get("<%=gridId%>");
		var selections = table.getSelections();
		if (selections.length == 0) {
			plm.showMessage({title:'错误提示', message:"没有可操作的数据", icon:'1'});
