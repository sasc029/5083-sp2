...
public class A3A5DispatchSigTaskOperSynMessageConvertImpl extends AbstractMessageConvert
	...
	public List<TransferObject> convertToA5Message(ObjectMessage om,
		...
		TransferObject object = super.createTransferObject(om.getMsgIID(), "电子分发任务操作同步消息", om.getMsgType(), "", null, sourceSite, targetSite, TransferConstant.REQTYPE_ASYN, reqParamMap);
	...
	private static Map<String, String> makeMessageMap(List<DataMap> dataList){
			...
			try {
				...
				logger.error("A3A5DispatchSigTaskOperSynMessageConvertImpl对象转换为reqParamMap失败",e);
				throw new RuntimeException("A3A5DispatchSigTaskOperSynMessageConvertImpl对象转换为reqParamMap失败",e);
			}
			return map;
	 }
	...
	private List<DataMap> distributeToDataMapList(Site sourceSite, Site targetSite,  
		...
		try {
			...
			logger.error("A3A5DispatchSigTaskOperSynMessageConvertImpl对象转换为DataMap失败",e);
			throw new RuntimeException("A3A5DispatchSigTaskOperSynMessageConvertImpl对象转换为DataMap失败",e);
		}
