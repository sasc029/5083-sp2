...
<script type="text/javascript">
    ...
    function doBindRole(returnReason, oids){
		...
		$.ajax({
			...
			error:function(){
				plm.endWait();
				plm.showMessage({title:'错误提示', message:"回退失败！", icon:'1'});
			}
		});
    }
	...
	function deleteDistributePaperTask(){
		...
		if (!plm.confirm("警告,删除发放单可能会删除该发放单上已添加的分发数据,是否继续删除?")) {
			return;
		}
		var oids = "";
		for(var i=0;i<data.length;i++){
		...
		$.ajax({
			...
			error:function(){
				plm.endWait();
				plm.showMessage({title:'错误提示', message:"删除问题失败！", icon:'1'});
			}
		});
	}
