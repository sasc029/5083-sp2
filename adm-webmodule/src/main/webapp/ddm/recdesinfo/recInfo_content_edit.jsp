...
function divSelected(obj){
	var tr = $(obj).parent();//当前title所在的tr对象
	//控制表格对象展示
	var tr1 = $(tr).next();
...
<!-- 分发数据 开始 -->
			...
			<table cellSpacing="0" cellPadding="0" width="100%" border="0" style="border-right:#9cbdff solid 1px;border-left:#9cbdff solid 1px;"><tr class="AvidmToolbar"><td> 
				...
				<table id="<%=gridId%>" class="pt-grid" singleSelect="false" checkbox="true" url="<%=gridUrl%>"
					...
					<thead>
						<th field="DISINFONAME"  width="110" tips="接收单位/人员">接收单位/人员</th>
						<th field="DISMEDIATYPE" width="100" title="" filterType="select" data="inSite" textField="id"  valueField="id">分发类型</th>
						<th field="DISINFONUM"  width="100" tips="分发份数">分发份数</th>
						<th field="RECOVERNUM"  width="100" tips="已回收份数">已回收份数</th>
						<th field="NEEDRECOVERNUM" 	 width="100"  tips="回收份数" editor="{type:'text'}">回收份数</th>
						<th field="NOTE" 		 width="241" tips="备注"     editor="{type:'text'}">备注</th>
					</thead>
				</table >
			</td></tr></table>
<!-- 分发数据 结束 -->	
	...
	function saveSubmitForm(){
		...
		for(var i = 0; i < selections.length; i++){
			...
			if((parseInt(disInfoNum)-parseInt(recoverNum))<parseInt(needRecoverNum)){
				plm.endWait();
				var mess = disInfoName+"-可回收的剩余份数："+(parseInt(disInfoNum)-parseInt(recoverNum))+"份";
