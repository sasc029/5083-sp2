...
<%
	...
	if (disObjList == null || disObjList.isEmpty()) {
		...
		orderType = ConstUtil.C_ORDERTYPE_3;
		orderTypeName="销毁发放单";
		...
		if (persit instanceof Iterated) {
			...
			for (int i = 0; i < itList.size(); i++) {
				...
				//同一个版本下--不同发放单
				List<DistributeOrderObjectLink> returnList =disObjService.historyDistributeObjectList(Helper.getOid(iter.getClassId(), iter.getInnerId()),"3");
				seriaNo+=returnList.size();
			}
		}
		number = disObject.getNumber() + ConstUtil.C_DISTRIBUTEORDER_3;
		name= disObject.getName() + ConstUtil.C_DISTRIBUTEORDER_3;
		/** 发放单与分发数据Link服务 */
...
<script type="text/javascript">
	...
	function saveSubmitForm() {
		...
		var options = {
				...
				url:"<%=contextPath%>/ddm/distribute/recDesInfo!addDesDistributeOrder.action",
				dataType:"json",
				type:"POST",
