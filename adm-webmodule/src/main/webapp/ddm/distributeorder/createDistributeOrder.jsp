...
<%@page import="com.bjsasc.plm.core.approve.Approved"%>
<%@page import="java.util.ArrayList"%>
<%@page import="com.bjsasc.plm.core.approve.ApproveOrder"%>
<%@page import="com.bjsasc.plm.core.approve.ApproveHelper"%>
...
<%
	...
	if (disObjList == null || disObjList.isEmpty()) {
		...
		//关于送审单补发发放单类型显示补发的修改（杨振州）
		if((obj instanceof ApproveOrder)){
			List<DistributeObject> collectApproveOrderRefResList = new ArrayList<DistributeObject>();
			//获取送审单下的子对象
			List<Approved> ptList = ApproveHelper.getApproveOrderService().getApprovedList((ApproveOrder) obj);	
			for (Persistable ptobj : ptList) {
					String classid=ptobj.getClassId();
					String innerid=ptobj.getInnerId();
					String oida=classid+":"+innerid;
					List<DistributeObject> disObjListData = disObjService.getDistributeObjectsByDataOid(oida);
					
				 	if(disObjListData!=null && disObjListData.size()>0){
						...
						int seriaNo = linkService.getDistributeOrderObjectLinkListByDistributeObjectOid(Helper.getOid(disObjListData.get(0)))
								.size();
	...
	String contextOid = "";
	Persistable obj1 = Helper.getPersistService().getObject(oid);
	if (obj1 instanceof Contexted) {
		ContextInfo contextInfo = ((Contexted)obj1).getContextInfo();
...
<script type="text/javascript">
	...
	function saveSubmitForm() {
		...
		var options = {
				...
				url:"<%=contextPath%>/ddm/distribute/distributeOrderHandle!createDistributeOrder.action",
				dataType:"json",
				type:"POST",
