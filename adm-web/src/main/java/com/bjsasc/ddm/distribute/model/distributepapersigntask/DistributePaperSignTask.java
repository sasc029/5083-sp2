package com.bjsasc.ddm.distribute.model.distributepapersigntask;
 ...
 */
public class DistributePaperSignTask extends DistributeTask{


	/** serialVersionUID */
	private static final long serialVersionUID = 6290320932825051328L;

	/**
	 * 构造方法。
	 */
	public DistributePaperSignTask() {
		setClassId(CLASSID);
	}

	private String taskType = ConstUtil.C_TASKYTPE_PAPERSIGN;

	public static final String CLASSID = DistributePaperSignTask.class.getSimpleName();
	...
	public ReturnReason getElecReturnReason() {
		if (lifeCycleInfo != null && ConstUtil.LC_REFUSE_SIGNED.getName().equals(lifeCycleInfo.getStateName())) {
			Object oid = getOid();
			String sql = "SELECT * FROM DDM_DIS_RETURN T WHERE T.TASKCLASSID || ':' || T.TASKID = ?";
			List<Map<String, Object>> returnReasonList = Helper.getPersistService().query(sql, oid);
			if (returnReasonList.size() > 0) {
				elecReturnReason = (ReturnReason)returnReasonList.get(0);
			}else{
	...
	public DistributePaperSignTask clone(DistributePaperSignTask task) {
	...
	public String toString() {
		return "DistributePaperSignTask [elecTaskType=" + elecTaskType + ", taskType=" + taskType + ", lifeCycleInfo="
